...
class ALNSSolver:
    ...
    def try_insert_task(self, solution, vessel, task_type, workload, berth):
        ...
        # 步骤1: 设置时间阈值TY - 参考当前占用块前一个占用的结束时间
        TY = tva
        berth_sequence = solution.berth_sequences.get(berth, [])
        for index, occ in enumerate(berth_sequence):
            if occ['vessel'] == vessel:
                if index > 0:
                    previous_end = berth_sequence[index - 1]['end']
                    TY = max(tva, previous_end)
                break
...
def test_full_alns_with_excel_data():
    ...
    # 从Excel文件创建实例
    excel_path = r"D:\0 博士阶段资料\毕业资料\Data_1.xlsx"
