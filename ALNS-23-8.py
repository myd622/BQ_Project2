...
class ALNSSolver:
    ...
    def global_reschedule_after_removal(self, solution, berth,  ):
        """
        场景1的步骤3：全局重新调度
        """
        instance = self.instance
    ...
    def create_new_occupation(self, temp_solution, vessel, berth, position_index, sub_occupations, time_threshold,
        ...
        # 插入到泊位序列中
        temp_solution.berth_sequences[berth].append(new_occupation)
        temp_solution.berth_sequences[berth].sort(key=lambda x: x['start'])

        # 更新决策变量
        temp_solution.Z[(vessel, berth)] = 1
        temp_solution.berth_occupation_start[(vessel, berth)] = start_time
        temp_solution.berth_occupation_end[(vessel, berth)] = start_time

        # 更新船舶移泊序列
        if vessel not in temp_solution.vessel_movements:
            temp_solution.vessel_movements[vessel] = []
        if berth not in temp_solution.vessel_movements[vessel]:
            temp_solution.vessel_movements[vessel].append(berth)

        # new_occupation['tasks'].append((task_type, workload))

        for occ in temp_solution.berth_sequences[berth]:
            if occ['vessel'] == vessel:
                occ['tasks'].append((task_type, workload))
                break

        self.update_vessel_movement_order(temp_solution, vessel)
    ...
    def insert_task_greedy(self, temp_solution, vessel, task_type, workload, time_threshold, time_threshold_S):
        ...
        for berth in possible_berths:
            # 步骤1: 检查互斥约束
            if not self.check_mutual_exclusion(temp_solution, vessel, task_type, berth):
                continue
        ...
        if best_berth is not None:
            temp_solution.__dict__.update(best_solution.__dict__)
            return True

        return False

    def try_insert_task(self, temp_solution, vessel, task_type, workload, berth):
        """尝试将任务插入到指定泊位（支持并行任务）"""
        instance = self.instance
        ...
        # 步骤2: 调用重新调度函数
        self.reschedule_after_insertion(temp_solution, vessel, task_type, berth, workload, TY)
        ...
        # 步骤4: 更新船舶结束时间和延迟
        self.update_vessel_end_times_and_delays(temp_solution)

        # 步骤5: 重新计算总成本
        temp_solution.cost = temp_solution.calculate_cost()

        # 步骤3: 检查优先级约束
        priority_edges = instance.get_priority_constraints(vessel)
        for (j_prime, j) in priority_edges:
            # 如果两个任务都已经调度
            if (vessel, j_prime) in temp_solution.task_end and (vessel, j) in temp_solution.task_start:
                if temp_solution.task_end[(vessel, j_prime)] > temp_solution.task_start[(vessel, j)]:
                    # 优先级约束不满足
                    print(
                        f"插入任务 ({vessel}, {task_type}) 违反优先级约束: {j_prime} 结束于 {temp_solution.task_end[(vessel, j_prime)]}, {j} 开始于 {temp_solution.task_start[(vessel, j)]}")
                    return False
        # print("2---------------------")

        return True

    def reschedule_after_insertion(self, temp_solution, vessel, task_type, berth, workload, original_start_time):
        """
        场景2：插入任务后的重新调度
        ...
        # 更新泊位分配
        temp_solution.X[(vessel, task_type, berth)] = 1
        temp_solution.task_to_berth[(vessel, task_type)] = berth
        temp_solution.Z[(vessel, berth)] = 1
        ...
        # 使用新的岸桥调度逻辑
        #end_time = self.schedule_berth_occupation(vessel, berth, tasks, temp_solution, start_time1)
        end_time = self.schedule_berth_occupation(vessel, berth, tasks, temp_solution, initial_start_time)

        scheduled_starts = [
            temp_solution.task_start[(vessel, t_type)]
            for t_type in occupation_task_types
            if (vessel, t_type) in temp_solution.task_start
        ]
        actual_start_time = min(scheduled_starts) if scheduled_starts else initial_start_time

        occupation['end'] = end_time
        occupation['start'] = actual_start_time
        temp_solution.berth_occupation_end[(vessel, berth)] = end_time
        temp_solution.berth_occupation_start[(vessel, berth)] = actual_start_time


        # 调整移泊顺序并按照开始时间排序泊位占用
        self.update_vessel_movement_order(temp_solution, vessel)
        berth_occupations = temp_solution.berth_sequences[berth]
        ...
        # 新增代码: 计算时间阈值SJYZ
        # 获取船舶v的移泊序列
        vessel_movements = temp_solution.vessel_movements.get(vessel, [])
        ...
        if current_berth_index == len(vessel_movements) - 1:  # 当前泊位是移泊序列中的最后一个
            ...
            # 获取下一个泊位的开始时间
            SJYZ = temp_solution.berth_occupation_start.get((vessel, next_berth), float('inf'))
        else:  # 当前泊位不在移泊序列中
        ...
        # a) 检查是否与前一个占用发生冲突
        if actual_start_time <= prev_occupation_end:
            if prev_occupation_end != float('-inf'):
                violation_threshold = min(violation_threshold, prev_occupation_end)
            return self.global_reschedule_after_insertion(temp_solution, berth, violation_threshold)

        # b) 判断Evb是否小于等于泊位b上船舶v的下一个船舶占用的开始时间
        if end_time >= next_occupation_start:
            if next_occupation_start != float('inf'):
                violation_threshold = min(violation_threshold, next_occupation_start)
            return self.global_reschedule_after_insertion(temp_solution, berth, violation_threshold)

        # c) 检查移泊时间约束
        if end_time + self.instance.tvc >= SJYZ:
            if SJYZ != float('inf'):
                violation_threshold = min(violation_threshold, SJYZ)
            return self.global_reschedule_after_insertion(temp_solution, berth, violation_threshold)
        ...
        for (j_prime, j) in priority_edges:
            if j_prime in occupation_task_type_set:
                successor_start = temp_solution.task_start.get((vessel, j))
                predecessor_end = temp_solution.task_end.get((vessel, j_prime))
    ...
    def global_reschedule_after_insertion(self, temp_solution, berth, threshold_time):
        """
        场景2的步骤2：全局重新调度
        """
        instance = self.instance
        ...
        for b in instance.berths:
            # 找到结束时间小于等于阈值时间的最大结束时间
            max_end_time = 0
            for occ in temp_solution.berth_sequences[b]:
                if occ['end'] <= threshold_time and occ['end'] > max_end_time:
                    max_end_time = occ['end']
            berth_availability[b] = max_end_time
        ...
        for crane in instance.Q:
            # 找到结束时间小于等于阈值时间的最大结束时间
            max_end_time = 0
            for task in temp_solution.crane_sequences[crane]:
                if task['end'] <= threshold_time and task['end'] > max_end_time:
                    max_end_time = task['end']
            crane_availability[crane] = max_end_time
        ...
        for b in instance.berths:
            for occ in temp_solution.berth_sequences[b]:
                if occ['end'] > threshold_time:
                    ...
                    for task in occ['tasks']:
                        task_t = task[0]
                        if (vessel, task_t) in temp_solution.task_start:
                            del temp_solution.task_start[(vessel, task_t)]
                        if (vessel, task_t) in temp_solution.task_end:
                            del temp_solution.task_end[(vessel, task_t)]

                    # 初始化岸桥变量
                    for task in occ['tasks']:
                        task_t = task[0]
                        if task_t in instance.JQ and (vessel, task_t) in temp_solution.task_to_crane:
                            crane = temp_solution.task_to_crane[(vessel, task_t)]
                            temp_solution.Y[(vessel, task_t, crane)] = 0
                            # 从岸桥序列中删除任务
                            crane_sequence = temp_solution.crane_sequences[crane]
                            for i, crane_task in enumerate(crane_sequence):
                                if crane_task['vessel'] == vessel and crane_task['task'] == task_t:
                                    del crane_sequence[i]
                                    break
                            del temp_solution.task_to_crane[(vessel, task_t)]
        ...
        for occ in SCDDS:
            ...
            # 查找泊位b
            for berth in instance.berths:
                if occ in temp_solution.berth_sequences[berth]:
                    b = berth
                    break
            ...
            berth_sequence = temp_solution.vessel_movements[vessel]

            berth_index = berth_sequence.index(b)

            if berth_index == 0:
                prev_berth_end = self.get_previous_berth_end_time(temp_solution, vessel, b)
            else:
                prev_berth_end = self.get_previous_berth_end_time(temp_solution, vessel, b) + instance.tvc
            ...
            for (j_prime, j) in priority_edges:
                if any(t[0] == j for t in tasks):
                    ...
                    predecessor_end = None
                    berth_prime = temp_solution.task_to_berth.get((vessel, j_prime))
                    if berth_prime is not None:
                        predecessor_end = temp_solution.berth_occupation_end.get((vessel, berth_prime))

                    if predecessor_end is None and (vessel, j_prime) in temp_solution.task_end:
                        predecessor_end = temp_solution.task_end[(vessel, j_prime)]
            ...
            # 调度占用
            end_time = self.schedule_berth_occupation(vessel, b, tasks, temp_solution, start_time)

            # 更新占用信息
            occ['start'] = start_time
            occ['end'] = end_time
            temp_solution.berth_occupation_start[(vessel, b)] = start_time
            temp_solution.berth_occupation_end[(vessel, b)] = end_time

            temp_solution.berth_sequences[b].sort(key=lambda x: x['start'])
            self.update_vessel_movement_order(temp_solution, vessel)
...
def create_instance_from_excel(excel_path):
    ...
    berths = {
        'b1': ['j1'],  # 只支持加油
        'b2': ['j2', 'j3','j4', 'j7'],  # 支持充电和加水
        'b3': ['j2', 'j3', 'j5', 'j6','j7'],  # 支持卸货、装货和加泥
    }
    '''
